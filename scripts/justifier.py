def justify(
    ...
    if enumerator:
        ...
        content = [
            [str(key + 1).zfill(rowsNumber)] + row for key, row in enumerate(content)
        ]
    ...
    justifiedParts = [
        [
            part.center(lineLen[key] + justsize, vertSign)
            for key, part in enumerate(item)
        ]
        for item in content
    ]
    justifiedColoredParts = [
        [colored(part, *colors_column[index]) for part in item]
        for index, item in enumerate(justifiedParts)
    ]
    content = [horSign.join(item) for item in justifiedColoredParts]

    line = "+".join(
        ["-" * len(item) for item in justifiedParts[0]]
    )  # with '+' in the cross
    if frame:
    ...
    if contentWidth > 2:
        line = "+" + "-" * (contentWidth - 2) + "+"
        sentence = (
            "+" + topbar[: contentWidth - 2].upper().center(contentWidth - 2, " ") + "+"
        )
        if frame:
            strTopbar = "\n".join([line, sentence])
        else:
